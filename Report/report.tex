...
\textbf{WebRTC} is a set of protocols and browser programming interfaces that let two machines exchange audio, video and data directly, in real time, with end-to-end encryption. In the browser, the entry point is the \texttt{RTCPeerConnection} object. \emph{WebRTC for the Curious}~\cite{curious}, an open community reference on the protocol suite, is a useful companion to the summary that follows.
...
\textbf{Encryption.} Once a candidate pair is selected, the two ends run a \textbf{DTLS} handshake directly over it. DTLS is essentially TLS adapted to work over an unreliable, packet-based transport like UDP. Each side authenticates the other not against a certificate authority, but by checking that the certificate presented during the handshake matches a fingerprint carried inside the SDP exchanged earlier, which is why the signaling channel, even though WebRTC does not standardize it, is trusted to deliver that document unmodified. The keys this handshake produces are then used to key \textbf{SRTP}, which encrypts and authenticates every media packet that follows.
