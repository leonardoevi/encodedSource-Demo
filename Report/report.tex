...
This gives the mechanism a clean, symmetric shape: compressed frames flow \emph{down}, from the application in Blink, across the browser's internal layer boundary, into the WebRTC library and onto the network, while the two control signals flow in the \emph{opposite direction}, from the network, through WebRTC's proxy encoder, back across that same boundary, up to the application in Blink.
